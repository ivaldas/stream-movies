...
export const addFilms = async (req, res) => {
  try {
    films_data.forEach(film => {insertFilm(film)})
...
function insertFilm(film) {
  ...
  const checkFilmQuery = 'SELECT COUNT(*) AS count FROM films WHERE programid = ?';
  db.query(checkFilmQuery, [film.programid], (err, result) => {
    ...
    if (result[0].count === 0) {
      const insertFilmQuery = "INSERT INTO films (file_path, full_path, movieyear, title, description, isepisode, isepisodic, seriesid, programid, starrating, mpaarating, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
      db.query(insertFilmQuery, [
        file_path,
        full_path,
        movieyear,
        title,
      ...
      (err, result) => {
        ...
        insertFilmCrew(directors, 'directors', 'filmdirectors', 'director_id', film_id)
        insertFilmCrew(writers, 'writers', 'filmwriters', 'writer_id', film_id)
