...
const streamFilm = async (req, res) => {
    ...
    const { id } = req.params;
    // Find film in DB
    const foundFilm = films_data.find((film) => String(film.programid) === String(id));
    ...
    const mp4FolderPath = resolve(folderPath, 'Plex Versions', 'Original Quality');
    ...
    if (mp4Files.length > 0) {
      ...
      const exact = mp4Files.find(file => basename(file) === `${baseFileName}.mp4`);
      const selected = exact ?? mp4Files.find(file => basename(file).startsWith(baseFileName)) ?? mp4Files[0];

      // Resolve safely
      const { path: safeMp4Path, file: mp4FileHandle } = await resolveVideoPath(selected, mp4FolderPath, ['.mp4']);
      console.log("Streaming MP4:", safeMp4Path);

      // Stream using file handle (optional) or just path
      return streamFilmByteRange(req, res, safeMp4Path, mp4FolderPath, mp4FileHandle);
    }
    ...
    if (fileExtension === '.mp4' || fileExtension === '.webm') {
      const { path: safePath, file: fileHandle } = await resolveVideoPath(basename(originalPath), folderPath, ['.mp4', '.webm']);
      return streamFilmByteRange(req, res, safePath, folderPath, fileHandle);
    } else if (fileExtension === '.mkv') {
      const { path: safeMkvPath, file: mkvFileHandle } = await resolveVideoPath(basename(originalPath), folderPath, ['.mkv'])
